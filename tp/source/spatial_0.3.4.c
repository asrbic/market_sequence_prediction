...
const char *IN_FILE_NAME = "/media/Data/datasets/KTH_Source/training_data/trainData100x20000.bin";//"./Data2/wang_s0_200K_randTranches.bin";
FILE *inFile;
const char *INIT_FILE_NAME = "/media/Data/datasets/KTH_Source/training_data/init_wang_s0_100x20K_randTranches.bin";
FILE *initFile;
const char *STATE_FILE_NAME = "/media/Data/datasets/KTH_Source/training_data/state_wang_s0_100x20K_randTranches.bin";
FILE *stateFile;
const char *CODES_FILE_NAME = "/media/Data/datasets/KTH_Source/training_data/colCodes_wang_s0_100x20K_randTranches.bin";//"./ColCodes/colCodes_wang_s0_200K_randTranches.txt";
FILE *codesFile;
...
//getPatchFromArray prototype declared - asrbic
void getPatchFromArray(int patchIndex, unsigned char* patchData);
void getPatch();
...
int main(int argc, char *argv[])
{
	...
	//MODIFIED CODE BY ASRBIC - READ FILE INTO MEMORY FOR USE IN getPatch()
	fprintf(stdout,"file size:%i",numInstance*PIXELS);
	unsigned char* inputData = calloc(sizeof(unsigned char),numInstance*PIXELS);
	fread(inputData,sizeof(unsigned char),numInstance*PIXELS,inFile);
	rewind(inFile);
	randomiseIndex();
	//END MODIFICATIONS
...
//printf("%d\t%d\n", cycle, iIndex);
				...
				getPatchFromArray(inputIndex[iIndex],inputData);
				//getPatch modified to take an index paramater and patchData array - asrbic
				//getPatch();
				senseInput();
				calculateOverlap();
				inhibitColumns();
...
//		if(sCalcKurtosis) calculateKurtosis();
	...
	//asrbic addition - free allocated memory for input array 
	free(inputData);
...
void getPatchFromArray(int patchIndex, unsigned char* patchData)
{
//new - reads from patchData
	memcpy(newinput,(patchData+(patchIndex*PIXELS)),PIXELS*sizeof(unsigned char));
...
{
	int temp,randIndex;
//new - asrbic - modified to be fully random
	for(int i = 0; i < numInstance;++i)
	{
		temp = inputIndex[i];
		randIndex=rand()%numInstance;
		inputIndex[i]=inputIndex[randIndex];
		inputIndex[randIndex]=temp;
	}
